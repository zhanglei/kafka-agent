...
import com.hncy58.kafka.consumer.handler.TiDBHandler;
 ...
 * @version 1.0
 * @date 2019年2月21日 上午11:23:33
 *
 */
public class ConsumerToTiDBApp {

	private static final Logger log = LoggerFactory.getLogger(ConsumerToTiDBApp.class);

	private static final String PROP_PREFIX = "kafka-to-tidb";

	private static String agentSvrName = PropsUtil.getWithDefault(PROP_PREFIX, "agentSvrName", "KafkaToTiDB");
	private static String agentSvrGroup = PropsUtil.getWithDefault(PROP_PREFIX, "agentSvrGroup", "KafkaToTiDBGroup");
	...
	private static String kafkaGroupId = PropsUtil.getWithDefault(PROP_PREFIX, "kafkaGroupId", "ConsumerToTiDBApp");
	...
	private static String tidbURL = PropsUtil.getWithDefault(PROP_PREFIX, "tidbURL",
			"jdbc:mysql://162.16.9.238:4000/?useUnicode=true&characterEncoding=UTF-8&allowMultiQueries=true");
	private static String tidbUserName = PropsUtil.getWithDefault(PROP_PREFIX, "tidbUserName", "root");
	private static String tidbPasswd = PropsUtil.getWithDefault(PROP_PREFIX, "tidbPasswd", "");
	private static int tidbInitialSize = Integer
			.parseInt(PropsUtil.getWithDefault(PROP_PREFIX, "tidbInitialSize", "5"));
	private static int tidbMaxTotal = Integer.parseInt(PropsUtil.getWithDefault(PROP_PREFIX, "tidbMaxTotal", "10"));
	private static int tidbMaxIdle = Integer.parseInt(PropsUtil.getWithDefault(PROP_PREFIX, "tidbMaxIdle", "5"));
	private static int tidbMaxWaitMillis = Integer
			.parseInt(PropsUtil.getWithDefault(PROP_PREFIX, "tidbMaxWaitMillis", "5000"));
	private static int tidbBatchSize = Integer.parseInt(PropsUtil.getWithDefault(PROP_PREFIX, "tidbBatchSize", "1000"));
	...
	public static void main(String[] args) {
		...
		ConsumerToTiDBApp app = new ConsumerToTiDBApp();
	...
	private void init(String[] args) {
		...
		try {
			...
			setHandler(new TiDBHandler(agentSvrName, agentSvrGroup, agentSvrType, localFileNamePrefix, tidbURL,
					tidbUserName, tidbPasswd, tidbInitialSize, tidbMaxTotal, tidbMaxIdle, tidbMaxWaitMillis,
					tidbBatchSize));
